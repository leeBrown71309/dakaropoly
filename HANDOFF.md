...
- **Comptes (branche `feature/user-accounts`)** : connexion Google facultative (Supabase Auth), pseudo unique sensible à la casse, photo facultative, tableau de bord des 20 dernières parties en ligne (terminées ou inachevées) avec leur détail, suppression du compte (anonymisé chez les autres). Parties enregistrées par la base (`games`, `game_seats`). Schéma appliqué le 24/09/2026. **Reste : test complet de bout en bout.**
- Corrigé : au lancement, les chaises sont renumérotées du pion vers le numéro de joueur (avant, des pions choisis hors ordre rendaient tout le monde spectateur).
